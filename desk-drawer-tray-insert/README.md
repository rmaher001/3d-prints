...
prints as two halves of 183.25 × 180.5 × 29 mm that sit side by side. The tray's
...
1. **Fit test first**, in any spare filament. Print both outlines, drop them into
   ...
   first fit test (2026-09-23, `CLEAR` 0.5 mm per side) sat about 1 mm loose on
   every side, so `CLEAR` is now -0.5: the halves are 1 mm per side bigger than
   that test, and 0.5 mm per side bigger than the tray as measured. If they bind,
